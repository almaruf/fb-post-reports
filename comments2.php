...
try {
    ...
    foreach($data['data'] as $p) {
        $t .= "{$p['message']};";// <br/><a target='_blank' href='?see_replies={$p['id']}'>See replies</a></td>";
        $t .= "{$p['from']['name']};";
        $t .= "{$p['from']['id']};";
        $t .= "<a target='_blank' href='http://facebook.com/{$p['from']['id']}'>http://facebook.com/{$p['from']['id']}</a>;";

        $date = DateTime::createFromFormat('Y-m-d\TH:i:sT', $p['created_time']);
        $t .= "{$date->format('d F y')}@{$date->format('H:i')}</br>";
    }
    echo $t;
} catch(Exception $e) {
    echo $e->getMessage();
